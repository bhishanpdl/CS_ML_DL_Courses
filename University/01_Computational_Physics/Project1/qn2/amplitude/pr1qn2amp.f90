...
!bp: clear; gfortran -w pr1qn2amp.f90 && ./a.out
!bp: clear; f90 pr1qn2amp.f90 && ./a.out

! integral of -0.4726* sin(16*x*sin(theta/2))*e^(-7.559*x) / sin(theta/2) from zero to infinity for theta = 3.139
! from Wolfram Alpha = -0.0241  478
! location: /home/bhishan/Copy/Fall2015/Computn/Homeworks/Bhishan/pr1/qn2/amplitude/*.dat
...
!        integration points
      ...
      filename='pr1qn2amp.dat'
      open(unit=kwrite,file=filename,status='unknown')
      
        do theta = 0.001,3.14,0.001
        
        !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
        !theta = 3.139  ! wolmfram value = -0.0241  478, change value of ng until it gets this value
        !do ng =1,90   ! from ng=90 i get converging value so i take ng = 90
        !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
...
!
      ...
      sum3=sum3 + f(theta,sxx(i))  *wxx(i)  !f(x)
      end do
...
double precision function f(theta,r)

       implicit none
       double precision theta,r

       f = -0.4726d0 * exp(r-7.559d0*r) * sin(16.d0 * r * sin(theta/2.d0)) / sin(theta/2.d0)
