!cmd: clear;f90 hw5qn2a.f90 && ./a.out


! usage    : program for the integration  of sqrt(1-x^2) between the limits -1 to 1
...
!      parameter (ng=8)
      ...
      open(kwrite, File = 'hw5qn2a.dat', Status = 'Unknown')
...
!
      sum1=0.d0
      do 20 i=1,nxmx
20       sum1 = sum1 + (sqrt((1 - (sxx(i))**2)))   *wxx(i)  ! function
...
!
      sum2 = 0.d0
      do  i=1,nxmx
         sum2 = sum2 +(sqrt((1 - (sxx(i))**2)))   *wxx(i)  ! function
         end do
...
!
      sum3 = 0.d0
      do  i=1,ng
      sum3 = sum3 +(sqrt((1 - (sxx(i))**2)))   *wxx(i)  ! function
   end do
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!

   ! Relative errors
   
   e1 = abs((sum1*2.d0-exact)/exact)
   e2 = abs((sum2*2.d0-exact)/exact)
   e3 = abs((sum3*2.d0-exact)/exact)
   
   
      write(kwrite,10001) nxmx, dx,  sum1*2.d0,sum2*2.d0,sum3*2.d0, e1,e2,e3 
   ...
   close(kwrite)
   stop 'data saved in hw5qn2a.dat'
   end program
