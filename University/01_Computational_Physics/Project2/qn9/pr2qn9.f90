...
! clear; f95 pr2qn9.f90 -llapack && ./a.out ; rm *~ a.out
...
double precision :: v,norm,mysum

parameter (ng=50)

double precision :: normalized(ng),kinetic(ng)
...
do i=1,ng
  normalized(i) = -vr(i,yes)/norm 
end do

!writing Kinetic Energy Operator H0 = p^2 / 2m
mysum = 0.d0
do i = 1,ng
    kinetic(i) = w1(i) * p(i) * p(i) * normalized(i) * normalized(i)
    mysum      = mysum + kinetic(i) 
end do 

mysum = mysum * 4.d0 * pi / mu

write(*,*) 'Total Kinetic Energy is = ', mysum
...
! integrating the potential vl(q,q') from -1 to 1 with gauleg subroutine
 double precision function v(qprime,q)
      ...
      double precision :: m,E,Eprime 
       ...
       parameter (Va = 985.3073d0) ! 1085.3073d0
