...
class KernelPerceptron(object):
    ...
    def fit(self, X, y):
        ...
        self.sv_y = y[sv]
        # 
        # print("temp[0:10] = {}".format(temp[0:10]))
        # print("sv[0] = {}".format(self.sv[0])) # sv = [[ 0.55160822  3.76833834]
        # print("alpha = {}".format(self.alpha)) # alpha = [ 12.  15.   3.
        # print("ind = {}".format(ind)) # ind = [  0   1  18  50
...
def test_kernel():
    num_data = 120
    # X1, y1, X2, y2 = gen_non_lin_separable_data(num_data)
    
    X1, y1, X2, y2 = gen_lin_separable_data(num_data)
    X_train, y_train = split_train(X1, y1, X2, y2)
    X_test, y_test = split_test(X1, y1, X2, y2)
    ...
    correct = np.sum(y_predict == y_test)
    print("%d out of %d predictions correct" % (correct, len(y_predict)))
    print("Accuracy = {:.2f} %".format(correct/ len(y_predict) * 100))
