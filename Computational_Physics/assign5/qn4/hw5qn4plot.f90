...
! cmd: clear; f90 hw5qn4plot.f90 && ./a.out
...
!        for small values of x
     ...
     &  +2.d0/567.d0*x**6
      ...
      xival=xend-xstart
      open(kwrite, File="hw5qn4plot.dat", status="unknown")

      write (kwrite,10000) '#log10(N)',  'exact',  'Simpson','Gauss-Legendre','Filon', &
                    & 'log10(eS)',  'log10(eG)',   'log10(eF)' 
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
	...
	j=1
      ...
      nxm=4.0d0*j+1
      nxmx=nxm/4.0d0
      nxmx=4*nxmx+1    ! for j=1, nxmx is 5
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  ...
  eF = abs((sum1-exact)/exact)
        ...
        ! note that log(0) is not defined !!
        if(eS /=0.00000.and. eG /=0.00000 .and. eG /=0.00000) then
        write(kwrite,20000) log10(dfloat(nxmx)), exact, sums, sumg, sum1, log10(eS),log10(eG),log10(eF)
        end if                  
      ...
      10000 format(T1,A10,2x, 7(A12,3x))
      20000 format(T1,F10.6,2x, 6(F12.7,3x),F16.6)    ! for plot
      
 close(kwrite)
      stop 'data saved in hw5qn4plot.dat'
      end
!**************************************************************
