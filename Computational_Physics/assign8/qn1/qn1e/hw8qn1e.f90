...
!bp: clear; f95 hw8qn1e.f90 && ./a.out >bhishan.dat

!
!        program to test Sobel sequence
...
implicit none
	...
	integer            :: i, j
    integer, parameter :: np=20, nplot=800
    integer            :: iseed(10), iflag, iter
	double precision   :: xx,xi(2)
    double precision   :: a(np), p1(nplot)
    integer, parameter :: kread=5, kwrite=6, kplot1=11
...
!       initialize sobel sequence   
      	...
      	!bp: when n is negative, routine internally initializes
        !bp: subroutine sobseq ( n, x,iseed)
                   call sobseq (-1,xx,iseed(iter+1))
 
      	write (kwrite,*) ' seed = ',iseed(1)  !bp seed =  234579
      	write (kwrite,*)
...
!        one-directional
      	...
      	write(kwrite,*) 'printing np=20 random numbers'
      	do i=1, np
           write (kwrite,*) a(i)
      	end do 
...
!       generate and print nplot random numbers for plot
      	open (kplot1, file = 'hw8qn1e.dat', status = 'unknown')
	      !write(kplot1,*)'#      r(2*i-1)            r(2*i)'

    
!        initialize sequence again 
      	iter=5
      	call sobseq (-1,xi,iseed(iter+1))
      
!        generate nplot points 
!        two-directional  
      	do i=1,nplot,2 
      	...
      	do i=1,nplot/2
       	   write (kplot1,10000) p1((2*i-1)),p1(2*i)   ! r(2*i-1)    r(2*i)
      	end do
