...
# -*- coding: utf-8 -*-#
"""
Softmax Regression Example from ulfl stanford.
...
class SoftmaxRegression(object):
    ...
    def softmaxCost(self, theta, data, labels):
        ...
        theta_grad = -numpy.dot(ground_truth - probabilities, numpy.transpose(data))
        theta_grad = theta_grad / data.shape[1] + self.lamda * theta
    ...
    def softmaxPredict(self, theta, data):
        ...
        print("data[0:].shape = {}".format(data[0:].shape)) # (784, 10000)
        theta    = theta.reshape(self.num_classes, self.input_size)        
        theta_x  = data.T @ theta.T
